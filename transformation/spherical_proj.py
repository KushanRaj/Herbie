...
class SphericalProjection:
    ...
    def __init__(self, img_H, img_W, fov_UP=-3, fov_DOWN=25, normalise=True, means=None, std=None):
        ...
        self._fov = abs(self._fov_UP)+abs(self._fov_DOWN)
    ...
    def _project_scan(self, scan):
        ...
        return pos, remissions, proj, proj_x, proj_y, order, proj_index, proj_mask
    
    def _project_labels(self, proj_index, proj_mask, labels):
        # set unwanted labels as zero because the first class is anyways ignored!
        proj_labels = np.zeros((self._img_H, self._img_W), dtype=np.int32)
        proj_labels[proj_mask] = labels[proj_index[proj_mask]]
        return proj_labels
    
    def __call__(self, data):
        # performs the transform and returns data
        data["pos"], data["remissions"], data["proj"], data["proj_x"], data["proj_y"], data["order"], data["proj_index"], data["proj_mask"] = self._project_scan(data["scan"])
        if self._normalise:
